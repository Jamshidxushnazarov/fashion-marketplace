...
import { card as Card } from "../../utils/brendsData"; // Import your data file
...
const UpcomingBrands: React.FC = () => {
    const [cards, setCards] = useState<BrandCard[]>(
        Card.map((item) => ({ ...item, id: item.id.toString() }))
    );
    ...
    return (
        <div className="container">
            ...
            <div className="flex flex-wrap justify-center gap-6 pt-10">
                {cards.map(
                    (
                        { id, name, bio, isVerified, avatar, coverImage, isFollowed },
                        index
                    ) => (
                        <div
                            key={id + index}
